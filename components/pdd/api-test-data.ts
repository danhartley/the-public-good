const getPledgesByFeatures = ({snapShot=null}) => {
    ...
    return new Promise<any>((resolve, reject) => {
        data 
            ? resolve({ ...data.find(d => d.snapShot === _snapShot), snapShots: data.map(d => d.snapShot) })
            : reject({
                error: 'No data'            
            });
    });
...
const getPledgesByValues = async ({snapShot=null}) => {
